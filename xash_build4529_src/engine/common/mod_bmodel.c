...
void Mod_PrintWorldStats_f( void )
{
	...
	for( i = 0; i < ARRAYSIZE_XASH( worldstats ); i++ )
	{
...
qboolean Mod_LoadBmodelLumps( const byte *mod_base, qboolean isworld )
{
	...
	// loading base lumps
	for( i = 0; i < ARRAYSIZE_XASH( srclumps ); i++ )
		Mod_LoadLump( mod_base, &srclumps[i], &worldstats[i], isworld ? (LUMP_SAVESTATS|LUMP_SILENT) : 0 );

	// loading extralumps
	for( i = 0; i < ARRAYSIZE_XASH( extlumps ); i++ )
		Mod_LoadLump( mod_base, &extlumps[i], &worldstats[ARRAYSIZE_XASH( srclumps ) + i], isworld ? (LUMP_SAVESTATS|LUMP_SILENT) : 0 );
...
qboolean Mod_TestBmodelLumps( const char *name, const byte *mod_base, qboolean silent )
{
	...
	// loading extralumps
	for( i = 0; i < ARRAYSIZE_XASH( extlumps ); i++ )
		Mod_LoadLump( mod_base, &extlumps[i], &worldstats[ARRAYSIZE_XASH( srclumps ) + i], flags );
