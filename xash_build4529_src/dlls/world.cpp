...
int CGlobalState::Save( CSave &save )
{
	...
	if ( !save.WriteFields( "GLOBAL", this, m_SaveData, ARRAYSIZE_XASH(m_SaveData) ) )
		return 0;
	
	pEntity = m_pList;
	for ( i = 0; i < m_listCount && pEntity; i++ )
	{
		if ( !save.WriteFields( "GENT", pEntity, gGlobalEntitySaveData, ARRAYSIZE_XASH(gGlobalEntitySaveData) ) )
			return 0;
...
int CGlobalState::Restore( CRestore &restore )
{
	...
	ClearStates();
	if ( !restore.ReadFields( "GLOBAL", this, m_SaveData, ARRAYSIZE_XASH(m_SaveData) ) )
		return 0;
	...
	{
		if ( !restore.ReadFields( "GENT", &tmpEntity, gGlobalEntitySaveData, ARRAYSIZE_XASH(gGlobalEntitySaveData) ) )
			return 0;
...
//
	...
	for ( int i = 0; i < ARRAYSIZE_XASH(gDecals); i++ )
