...
int CBaseMonster::Save( CSave &save )
{
	if ( !CBaseToggle::Save(save) )
		return 0;
	return save.WriteFields( "CBaseMonster", this, m_SaveData, ARRAYSIZE_XASH(m_SaveData) );
}
...
int CBaseMonster::Restore( CRestore &restore )
{
	if ( !CBaseToggle::Restore(restore) )
		return 0;
	int status = restore.ReadFields( "CBaseMonster", this, m_SaveData, ARRAYSIZE_XASH(m_SaveData) );
...
void CBaseMonster::ReportAIState( void )
{
	...
	if ( (int)m_MonsterState < ARRAYSIZE_XASH(pStateNames) )
