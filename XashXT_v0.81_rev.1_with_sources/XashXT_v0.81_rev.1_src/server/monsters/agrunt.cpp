...
void CAGrunt :: PrescheduleThink ( void )
{
	if ( ShouldSpeak() )
	{
		if ( m_flNextWordTime < gpGlobals->time )
		{
			...
			do
			{
				num = RANDOM_LONG(0,ARRAYSIZE_XASH(pIdleSounds)-1);
...
void CAGrunt :: DeathSound ( void )
{
	...
	EMIT_SOUND ( ENT(pev), CHAN_VOICE, pDieSounds[RANDOM_LONG(0,ARRAYSIZE_XASH(pDieSounds)-1)], 1.0, ATTN_NORM );
}
...
void CAGrunt :: AlertSound ( void )
{
	...
	EMIT_SOUND ( ENT(pev), CHAN_VOICE, pAlertSounds[RANDOM_LONG(0,ARRAYSIZE_XASH(pAlertSounds)-1)], 1.0, ATTN_NORM );
}
...
void CAGrunt :: AttackSound ( void )
{
	...
	EMIT_SOUND ( ENT(pev), CHAN_VOICE, pAttackSounds[RANDOM_LONG(0,ARRAYSIZE_XASH(pAttackSounds)-1)], 1.0, ATTN_NORM );
}
...
void CAGrunt :: PainSound ( void )
{
	...
	EMIT_SOUND ( ENT(pev), CHAN_VOICE, pPainSounds[RANDOM_LONG(0,ARRAYSIZE_XASH(pPainSounds)-1)], 1.0, ATTN_NORM );
}
...
void CAGrunt :: Precache()
{
	...
	for ( i = 0; i < ARRAYSIZE_XASH( pDieSounds ); i++ )
...
Schedule_t	slAGruntCombatFail[] =
{
	{
		tlAGruntCombatFail,
		ARRAYSIZE_XASH ( tlAGruntCombatFail ),
		bits_COND_CAN_RANGE_ATTACK1	|
		bits_COND_CAN_MELEE_ATTACK1,
		0,
		"AGrunt Combat Fail"
...
Schedule_t slAGruntStandoff[] = 
{
	{
		tlAGruntStandoff,
		ARRAYSIZE_XASH ( tlAGruntStandoff ),
		bits_COND_CAN_RANGE_ATTACK1		|
		bits_COND_CAN_MELEE_ATTACK1		|
		bits_COND_SEE_ENEMY				|
		bits_COND_NEW_ENEMY				|
...
{
	{
		...
		ARRAYSIZE_XASH ( tlAGruntSuppressHornet ),
		0,
		0,
...
Schedule_t	slAGruntRangeAttack1[] =
{
	{ 
		...
		ARRAYSIZE_XASH ( tlAGruntRangeAttack1 ), 
		bits_COND_NEW_ENEMY			|
...
Schedule_t	slAGruntHiddenRangeAttack[] =
{
	{ 
		...
		ARRAYSIZE_XASH ( tlAGruntHiddenRangeAttack1 ), 
		bits_COND_NEW_ENEMY			|
		bits_COND_HEAVY_DAMAGE		|
		bits_COND_HEAR_SOUND,
...
Schedule_t	slAGruntTakeCoverFromEnemy[] =
{
	{ 
		...
		ARRAYSIZE_XASH ( tlAGruntTakeCoverFromEnemy ), 
		bits_COND_NEW_ENEMY,
		0,
...
Schedule_t	slAGruntVictoryDance[] =
{
	{ 
		...
		ARRAYSIZE_XASH ( tlAGruntVictoryDance ), 
		bits_COND_NEW_ENEMY		|
...
Schedule_t	slAGruntThreatDisplay[] =
{
	{ 
		...
		ARRAYSIZE_XASH ( tlAGruntThreatDisplay ), 
		bits_COND_NEW_ENEMY			|
		bits_COND_LIGHT_DAMAGE		|
		bits_COND_HEAVY_DAMAGE,
